  ...
  await yargs(hideBin(process.argv))
    ...
    .command('upload <lambdas...>', 'Bundle, archive and upload to the given lambda functions', (yargs) => {
      return yargs
        ...
        .option('region', { alias: 'r', type: 'string', default: argsDefatuls.region, description: 'set the region of the lambda functions' })
        .example([
    ...
    .options({
      'e': { alias: 'entry', type: 'string', default: argsDefatuls.entry, description: 'The name of the entry file containing the lambda handler(s)' },
      'x': { alias: 'external', type: 'array', default: argsDefatuls.external, description: 'Exclude these libraries from bundling, for example libraries that you already load in a layer' },
      'm': { alias: 'metafile', type: 'boolean', default: argsDefatuls.metafile, description: 'Generate a meta.json file that can be used to analyze the bundle' },
    })
    .example([
